    ...
    try{
        ...
        if(admin){
            ...
            var mailOptions={
                ...
                text:`here is your link to create new password for book store application ${req.body.url}/${req.body.email}`
            };
...
router.put('/newPassword/:email',async (req,res,next)=>{
    try{
        const admin =await authenticateAdmin(req.params.email);
        if(admin){
